...
%     cd(Dir_GSim);
%     if(userFilePref == 0)       
%         [filename,pathname]=uiputfile('*.asc','Save the Output Classification of Difference to a File');    % Select final file name
%         CoD_file_name=[pathname filename '.asc'];   
%     else
        CoD_file_name= strcat(Dir_GSim, '/','_CoD.asc');
%     end
        ...
        fid1= fopen('temp.txt', 'w');
            ...
            for i=1:ny;              % Begin main gridcell loop (rows)
                for j=1:nx;          % Begin gridcell loop (collumns)
                    if ((CoDGrid(j,i) == nodata) | (CoDGrid(j,i) == 0));
                        fprintf(fid1, '-9999 ');  
                    else    
                        fprintf(fid1, '%u ',CoDGrid(j,i));   
                    end
