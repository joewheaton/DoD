...
if(userFilePref == 0)
    ...
    table_file_name= strcat(Dir_Run, '/P6DoD_ElevDist.csv');
end    
    
fid20 = fopen(table_file_name, 'w');    %write a file based on user's specificaitons
        
fprintf(fid20,'Upper Elevation Range (m), Lower Elevation Range (m), Gross Total Area (m2), Gross Total Volume (m3), Gross Number of Cells, CI minLoD Total Area (m2), CI minLoD Total Volume (m3), CI minLoD Number of Cells\n');
