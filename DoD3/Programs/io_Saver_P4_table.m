...
if(userFilePref == 0)
    ...
    table_file_name= strcat(Dir_Run, '/P4DoD_ElevDist.csv');
end    
    
fid20 = fopen(table_file_name, 'w');    %write a file based on user's specificaitons
   
fprintf(fid20,'Upper Elevation Range (m), Lower Elevation Range (m), Gross Total Area (m2), Gross Total Volume (m3), Gross Number of Cells, FIS minLoD Total Area (m2), FIS minLoD Total Volume (m3), FIS minLoD Number of Cells, SC minLoD Total Area (m2), SC minLoD Total Volume (m3), SC minLoD Number of Cells \n');
